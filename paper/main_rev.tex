...
In this methodological part, we are selecting $2$ clusters in each stratum so that overall we select $10$ primary units (clusters).
...
In this particular setting, we get that the sample mean $\hat{m}(z)_{cl2(PP)}=7009.67$ and sample variance 

$\hat{var}(\hat{m}(z)_{cl2(PP)})=258130.64$. We used the following formulae (both taken from \cite{thompson2012sampling}): 
\[
...
To explain the naming. We select clusters by stratified cluster random sampling, that is we consider each stratum as a population itself, and we select $CinS$ clusters in each stratum (as we have $5$ strata, we have decided to keep the total number of clusters the same throughout all $3$ two-stage cluster random sampling approaches. That means that when we are selecting $10$ primary units in $A$ and $B$, we select two primary units (clusters) in each stratum, and correspondingly, we select three clusters per stratum in case we select $15$ primary units. Thus, we consider each stratum as an independent population, select clusters in this stratum by simple random sampling, and then select population units again by simple random sampling. Then we use the upcoming Formula~\ref{eq_t_st} and Formula~\ref{eq_var_t_st} to calculate the mean estimator and sample variance of the mean estimator.
...
\begin{equation}\label{eq_var_t_st}
...
Lastly, in Table~\ref{tab:bias_variance_empirical} we can see the comparison of selected methods when calculating bias and variance manually from $100$ repetitions (with different individual selections). We can see that the variance in simple random sampling remains relatively small compared to all other methods (when the number of secondary units is larger than 2). In case the number of secondary units is $2$, then the variance in simple random sampling is much worse but in stratified two-stage cluster random sampling is the best among all methods. 

\begin{table}[htbp]
\centering
\begin{tabular}{rrrrrrrrrr}
\hline
psu & ssu & Bias\_A & Var\_A & Bias\_B & Var\_B & Bias\_C & Var\_C & Bias\_D & Var\_D \\
\hline
10 & 2  &  -72.9 & 175852.1 &  80.8  & 327659.9 &  -84.5 & 161401.1 &  29.4  & 192724.7 \\
10 & 5  &   31.8 & 188570.3 &  13.5  & 179476.8 &   44.5 &  98887.6 & -31.6  &  74481.9 \\
10 & 10 &  -72.5 & 139025.1 & -42.3  & 163728.1 &  -32.1 & 134355.3 &   7.5  &  43570.7 \\
15 & 2  &  -57.3 & 118584.0 &  38.5  & 133116.9 &   13.9 & 110063.2 &  -5.5  & 147191.9 \\
15 & 5  &    0.9 &  50828.2 &  15.5  &  90145.3 &   43.0 &  63819.9 &  -1.4  &  44992.6 \\
15 & 10 &  -50.4 &  40002.4 & -20.6  &  67825.1 &   21.9 &  46982.5 & -10.2  &  21912.3 \\
\hline
\end{tabular}
\caption{Bias and variance results calculated manually}
\label{tab:bias_variance_empirical}
\end{table}
